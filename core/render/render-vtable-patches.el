;;; render-vtable-patches.el --- Centred vtable column headers  -*- lexical-binding: t; -*-
...
;; ── empty-objects guard ──────────────────────────────────────────────────────
;; `make-vtable' with an empty `:objects' crashes: vtable computes each column's
;; width via `(seq-max (mapcar #'string-pixel-width VALUES))', and `seq-max' on
;; the empty value list bottoms out in `(max)' --- "Wrong number of arguments:
;; max, 0".  A view whose data source is momentarily empty (e.g. `whatsapp/*'
;; while wasabi's cache is still warming, or any org-ql query with no matches)
;; would otherwise take down the whole render.  We intercept that one case and
;; insert a subtle empty-state line instead of building a doomed table.

(defun aq--make-vtable-guard-empty (orig &rest kwargs)
  "Around-advice on `make-vtable': skip the table (insert an empty-state line
and return nil) when `:objects' is empty, else defer to ORIG with KWARGS.
Callers use `make-vtable' only for its insertion side-effect, never its return."
  (if (and (plist-member kwargs :objects)
           (null (plist-get kwargs :objects)))
      (progn (insert (propertize "  (nothing here right now)\n"
                                 'face '(:foreground "gray60" :slant italic)))
             nil)
    (apply orig kwargs)))

(advice-add 'make-vtable :around #'aq--make-vtable-guard-empty)

(provide 'render-vtable-patches)
;;; render-vtable-patches.el ends here
