;;; org-timestamp.el --- Clicking an org timestamp opens an actionable-query view  -*- lexical-binding: t; -*-
...
(defun actionable-query-enable-timestamp-views ()
  ...
  (interactive)
  (advice-add 'org-follow-timestamp-link :around #'org-timestamp--follow-advice))

(defun actionable-query-disable-timestamp-views ()
  "Restore Org's native one-day agenda on timestamp click."
  (interactive)
  (advice-remove 'org-follow-timestamp-link #'org-timestamp--follow-advice))
