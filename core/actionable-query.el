...
;;   fetchers.el  — generic async shell + parse helpers.
;;   agenda/      — org-agenda keys, scheduler, :org-ql preset, timestamps.
...
;;
;; Every module lives in `core/' itself or under one of `render/',
;; `state/', `interaction/', `point-async/', `agenda/'.  Add each to
;; `load-path' (relative to *this* file's directory) so the `(require
;; ...)` chain below resolves regardless of how the package was
;; installed ---direct `load-file' from init.org, `package.el',
;; `straight.el', whatever.

(let ((root (file-name-directory (or load-file-name buffer-file-name))))
  ;; `.' is core/ itself; the subdirs carry the kind-grouped modules
  ;; (`agenda/' holds agenda, scheduler, org-ql preset, org-timestamp).
  (dolist (sub '("." "render" "state" "interaction" "point-async" "agenda"))
...
(require 'render-vtable-patches)    ; advice on vtable--insert-header-line
(require 'fetchers)         ; generic async fetch + parse helpers
(require 'state-region-ctx)         ; aq-region-ctx struct, aq--message
(require 'state-cache)              ; async object cache, aq--obj-id
(require 'state-dismissal)          ; snooze + heart + footers
(require 'state-loading)            ; spinner + auto-refresh timer
(require 'interaction-row-reorder)  ; M-<up> / M-<down>
(require 'interaction-help-echo)    ; help-echo + org-marker hooks
(require 'interaction-bulk)         ; mark / unmark / bulk transient
(require 'interaction-popup)        ; transient `?' popup
(require 'interaction-filters)      ; `=' column filters
(require 'interaction-edit)         ; `e' editable-cell edit
(require 'interaction-keys)         ; vtable + region keymaps
(require 'render-grouped)           ; multi-table grouped layout
(require 'render-splice)            ; splice into host buffer
...
(define-C-x-C-e-extension actionable-query-defview (head-or-name &rest _rest)
  ;; `:named' MUST precede the docstring --- the macro parses [:named NAME]
  ;; *before* an optional docstring; a docstring-first ordering leaves the
  ;; `:named' clause unstripped in the body, where it evaluates the alias
  ;; symbol and signals a void-variable error on every `C-x C-e'.
...
(require 'scheduler)
(require 'agenda)
;; `org-ql.el' registers the `:org-ql' defview preset; its feature is named
;; `actionable-query-org-ql' (not `org-ql', that's the third-party lib), so it
;; can't be `require'd by basename --- load it directly.
(require 'actionable-query-org-ql nil t)
(unless (featurep 'actionable-query-org-ql)
  (load (expand-file-name "agenda/aq-org-ql" (file-name-directory (or load-file-name buffer-file-name)))))
