...
(defun whatsapp--jid->phone (jid)
  "Strip @s.whatsapp.net from JID and format as +NNNNN."
  (let ((digits (replace-regexp-in-string "@.*" "" (or jid ""))))
    (if (string-empty-p digits) "" (concat "+" digits))))

(defun whatsapp--wasabi-contact->plist (entry)
  ;; entry → (:jid :name :phone :snippet :timestamp)
  (with-current-buffer (wasabi--buffer)
    (let* ((jid   (symbol-name (car entry)))
           (info  (cdr entry))
           (name  (or (map-elt info :full-name) (map-elt info :push-name) jid))
           (chat  (seq-find (lambda (c) (string= (map-elt c :chat-jid) jid))
    ...
    (:name "Last seen"
           :getter    (lambda (o &rest _)
                        (let ((ts (plist-get o :timestamp)))
                          (if ts (whatsapp--format-ts ts)
                            (concat "☎ " (or (plist-get o :phone) "")))))
